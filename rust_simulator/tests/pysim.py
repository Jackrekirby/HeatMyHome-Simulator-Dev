...
# Added timer
# Added CSV Output
# Added CML parser (including latitude and longitude, removing import pgeocode)
# Changed assets files to rust format
# Added Quick Run option
...
if Location_Input[0:2] == "AL" or Location_Input[0:4] == "CM21" or Location_Input[0:4] == "CM22" or\
    ...
    EPC_Outside_Temp = [4.8, 5.2, 6.4, 8.4, 10.9, 13.5, 15.0, 14.9, 13.1, 10.0, 7.2, 4.7]  # Region 21
...
File_Name = "lat_" + Lat_Rounded + "_lon_" + Lon_Rounded + ".csv"
Outside_Temp = []
Solar_Irradiance = []
Coldest_Outside_Temp = 5  # Initial set point, then reduced depending on location weather

temp_file_path = ASSETS_DIR + "outside_temps/" + File_Name
# print(f"temp_file_path: {temp_file_path}")
if os.path.isfile(temp_file_path):
    Weather_File = open(temp_file_path, "r")
    Weather_Data = csv.reader(Weather_File)
    for Row in Weather_Data:
        Outside_Temp.append(float(Row[0]))
        if (float(Row[0])) < Coldest_Outside_Temp:
            Coldest_Outside_Temp = (float(Row[0]))
    Weather_File.close()
else:
    print("Cannot find temperature file for that location")
    exit()

irradiance_file_path = ASSETS_DIR + "solar_irradiances/" + File_Name
if os.path.isfile(irradiance_file_path):
    Weather_File = open(irradiance_file_path, "r")
    Weather_Data = csv.reader(Weather_File)
    for Row in Weather_Data:
        Solar_Irradiance.append(float(Row[0]))
    Weather_File.close()
else:
    print("Cannot find irradiance file for that location")
    exit()

Agile_Tariff = []  # Octopus Agile tariff 2020
Agile_File = open(f"{ASSETS_DIR}agile_tariff.csv")
Agile_Data = csv.reader(Agile_File)
for Row in Agile_Data:
    Agile_Tariff.append(float(Row[0]))
Agile_File.close()
...
for p in range(2):
    ...
    if p == 0:  # HP
        ...
        HP_DHW_Demand = DHW_Total
        HP_Space_Demand = (Demand_Total - DHW_Total)
        ...
        Boiler_DHW_Demand = DHW_Total
        Boiler_Space_Demand = (Demand_Total - DHW_Total)
...
# CSV FILE GENERATION
HeatOptionNames = ['ElectricResistanceHeating', 'AirSourceHeatPump', 'GroundSourceHeatPump']
SolarOptionNames = ['None','PhotoVoltaics','FlatPlate','EvacuatedTube','PhotoVoltaicsWithFlatPlate','PhotoVoltaicsWithEvacuatedTube','PhotoVoltaicThermalHybrid']
TariffNames = ['FlatRate','Economy7','BulbSmart','OctopusGo','OctopusAgile']

f = open(Save_Location, "w")

f.write("postcode,latitude,longitude,thermostat_temperature,num_occupants,house_size,epc_space_heating,tes_volume_max\n")
f.write(f"{Location_Input},{Latitude},{Longitude},{Temp},{Occupants_Num},{House_Size},{EPC_Space_Heating},{TES_Volume_Maximum}\n")

f.write(f"thermal_transmittance,{Thermal_Transmittance:.2f},optimised_epc_demand,{Optimised_EPC_Demand:.0f}\n")

f.write("heat_tech,hot_water_demand,space_demand,total_demand,max_hourly_demand\n")
f.write(f"hp,{HP_DHW_Demand:.0f},{HP_Space_Demand:.0f},{HP_Demand_Total:.0f},{HP_Max_Demand:.4f}\n")
f.write(f"boiler,{Boiler_DHW_Demand:.0f},{Boiler_Space_Demand:.0f},{Boiler_Demand_Total:.0f},{Boiler_Max_Demand:.4f}\n")

f.write("heat_option,solar_option,pv_size,solar_thermal_size,tes_volume,tariff,operational_expenditure,capital_expenditure,net_present_cost,operational_emissions\n")
for s in Optimum_TES_and_Tariff_Specifications:
    f.write(f"{HeatOptionNames[s[2]]},{SolarOptionNames[s[3]]},{s[4]},{s[5]},{s[6]},{TariffNames[s[9]]},{s[0]:.0f},{s[1]:.0f},{s[7]:.0f},{s[8]:.0f}\n")

f.write(f"HydrogenBoiler,Grey,,,,,{H2_Grey_Boiler_OpEx:.0f},{H2_Boiler_CapEx:.0f},{H2_Grey_Boiler_NPC:.0f},{H2_Grey_Boiler_OpEx:.0f}\n")
f.write(f"HydrogenBoiler,Blue,,,,,{H2_Blue_Boiler_OpEx:.0f},{H2_Boiler_CapEx:.0f},{H2_Blue_Boiler_NPC:.0f},{H2_Blue_Boiler_OpEx:.0f}\n")
f.write(f"HydrogenBoiler,Green,,,,,{H2_Green_Boiler_OpEx:.0f},{H2_Boiler_CapEx:.0f},{H2_Green_Boiler_NPC:.0f},{H2_Green_Boiler_OpEx:.0f}\n")
f.write(f"HydrogenFuel_Cell,Grey,,,,,{H2_Grey_Fuel_Cell_OpEx:.0f},{H2_Fuel_Cell_CapEx:.0f},{H2_Grey_Fuel_Cell_NPC:.0f},{H2_Grey_Fuel_Cell_OpEx:.0f}\n")
f.write(f"HydrogenFuel_Cell,Blue,,,,,{H2_Blue_Fuel_Cell_OpEx:.0f},{H2_Fuel_Cell_CapEx:.0f},{H2_Blue_Fuel_Cell_NPC:.0f},{H2_Blue_Fuel_Cell_OpEx:.0f}\n")
f.write(f"HydrogenFuel_Cell,Green,,,,,{H2_Green_Fuel_Cell_OpEx:.0f},{H2_Fuel_Cell_CapEx:.0f},{H2_Green_Fuel_Cell_NPC:.0f},{H2_Green_Fuel_Cell_OpEx:.0f}\n")

f.write(f"BiomassBoiler,,,,,,{Biomass_OpEx:.0f},{Biomass_CapEx:.0f},{Biomass_NPC:.0f},{Biomass_OpEx:.0f}\n")
f.write(f"Gas_Boiler,,,,,,{Gas_Boiler_OpEx:.0f},{Gas_Boiler_CapEx:.0f},{Gas_Boiler_NPC:.0f},{Gas_Boiler_OpEx:.0f}\n")
# [(Operational_Costs_Peak + Operational_Costs_Off_Peak), CapEx, HP_Option, Solar_Option,
#  PV_Size, Solar_Thermal_Size, TES_Volume_Current, Net_Present_Cost_Current,
#  Operation_Emissions, Tariff]
f.close()
