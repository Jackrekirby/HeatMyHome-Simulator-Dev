...
namespace heatninja2d {
    ...
    std::string run_simulation(const double thermostat_temperature, const double latitude, const double longitude, const int num_occupants, const double house_size, const std::string& postcode, const int epc_space_heating, const double tes_volume_max, const SimulationOptions& simulation_options) {
        ...
        heatninja2d::simulation_options = simulation_options;
        constexpr int double_print_precision = 2;
        std::cout << "===== Simulation Started =====\n";
        std::cout << "--- Input Parameters ---\n";
        std::cout << "thermostat_temperature: " << double_to_string(thermostat_temperature, double_print_precision) << '\n';
        std::cout << "latitude: " << double_to_string(latitude, double_print_precision) << '\n';
        std::cout << "longitude: " << double_to_string(longitude, double_print_precision) << '\n';
        std::cout << "num_occupants: " << num_occupants << '\n';
        std::cout << "house_size: " << double_to_string(house_size, double_print_precision) << '\n';
        ...
        const std::array<double, 24> erh_hourly_temperatures_over_day = calculate_erh_hourly_temperature_profile(thermostat_temperature);
        const std::array<double, 24> hp_hourly_temperatures_over_day = calculate_hp_hourly_temperature_profile(thermostat_temperature);

        constexpr std::array<double, 12> monthly_solar_declinations = { -20.7f, -12.8f, -1.8f, 9.8f, 18.8f, 23.1f, 21.2f, 13.7f, 2.9f, -8.7f, -18.4f, -23.0f };
        const std::array<double, 12> monthly_solar_height_factors = calculate_monthly_solar_height_factors(latitude, monthly_solar_declinations);

        constexpr std::array<double, 12> dhw_monthly_factors = { 1.10f, 1.06f, 1.02f, 0.98f, 0.94f, 0.90f, 0.90f, 0.94f, 0.98f, 1.02f, 1.06f, 1.10f };
        const std::array<double, 12> monthly_cold_water_temperatures = calculate_monthly_cold_water_temperatures(latitude);

        const std::array<double, 12> monthly_solar_gain_ratios_north = calculate_monthly_solar_gain_ratios_north(monthly_solar_height_factors);
        const std::array<double, 12> monthly_solar_gain_ratios_south = calculate_monthly_solar_gain_ratios_south(monthly_solar_height_factors);

        constexpr std::array<double, 24> hot_water_hourly_ratios = { 0.025f, 0.018f, 0.011f, 0.010f, 0.008f, 0.013f, 0.017f, 0.044f, 0.088f, 0.075f, 0.060f, 0.056f, 0.050f, 0.043f, 0.036f, 0.029f, 0.030f, 0.036f, 0.053f, 0.074f, 0.071f, 0.059f, 0.050f, 0.041f };

        const std::vector<double> hourly_outside_temperatures_over_year = import_weather_data("outside_temps", latitude, longitude);
        const std::vector<double> hourly_solar_irradiances_over_year = import_weather_data("solar_irradiances", latitude, longitude);

        const double average_daily_hot_water_volume = calculate_average_daily_hot_water_volume(num_occupants);

        constexpr int hot_water_temperature = 51;

        const double solar_gain_house_factor = calculate_solar_gain_house_factor(house_size);

        const double epc_body_gain = calculate_epc_body_gain(house_size);

        const int region_identifier = calculate_region_identifier(postcode);
        const std::array<double, 12> monthly_epc_outside_temperatures = calculate_monthly_epc_outside_temperatures(region_identifier);
        const std::array<int, 12> monthly_epc_solar_irradiances = calculate_monthly_epc_solar_irradiances(region_identifier);

        const std::array<double, 12> monthly_incident_irradiance_solar_gains_north = calculate_monthly_incident_irradiance_solar_gains_north(monthly_solar_gain_ratios_north, monthly_epc_solar_irradiances);
        const std::array<double, 12> monthly_incident_irradiance_solar_gains_south = calculate_monthly_incident_irradiance_solar_gains_south(monthly_solar_gain_ratios_south, monthly_epc_solar_irradiances);

        const std::array<double, 12> monthly_solar_gains_south = calculate_monthly_solar_gains_south(monthly_incident_irradiance_solar_gains_south, solar_gain_house_factor);
        const std::array<double, 12> monthly_solar_gains_north = calculate_monthly_solar_gains_north(monthly_incident_irradiance_solar_gains_north, solar_gain_house_factor);
        const double heat_capacity = calculate_heat_capacity(house_size);
        const double body_heat_gain = calculate_body_heat_gain(num_occupants);
        ...
        const double discount_rate = 1.035f; // 3.5% standard for UK HMRC
        const int npc_years = 20;

        const double coldest_outside_temperature_of_year = calculate_coldest_outside_temperature_of_year(latitude, longitude);
        const double ground_temp = calculate_ground_temperature(latitude);
        const int tes_range = calculate_tes_range(tes_volume_max);
        const int solar_maximum = calculate_solar_maximum(house_size);
        const double house_size_thermal_transmittance_product = calculate_house_size_thermal_transmittance_product(house_size, dwelling_thermal_transmittance);
        const double cumulative_discount_rate = calculate_cumulative_discount_rate(discount_rate, npc_years);
        const std::array<double, 12> monthly_roof_ratios_south = calculate_roof_ratios_south(monthly_solar_declinations, latitude);
        constexpr double u_value = 1.30f / 1000; // 0.00130 kW / m2K linearised from https ://zenodo.org/record/4692649#.YQEbio5KjIV &
        const std::vector<double> agile_tariff_per_hour_over_year = import_per_hour_of_year_data("assets/agile_tariff.csv");
        ...
        #ifndef EM_COMPATIBLE
        print_optimal_specifications(optimal_specifications, double_print_precision);
    ...
    double round_coordinate(const double coordinate) {
        double rounded_coordinate = std::roundf(coordinate * 2) / 2;
    ...
    std::vector<double> import_weather_data(const std::string& data_type, const double latitude, const double longitude) {
        ...
        const double rounded_latitude = round_coordinate(latitude);
        const double rounded_longtitude = round_coordinate(longitude);
        const std::string filename = "assets/" + data_type + "/lat_" + double_to_string(rounded_latitude, 1) + "_lon_" + double_to_string(rounded_longtitude, 1) + ".csv";
    ...
    std::array<double, 12> calculate_monthly_solar_height_factors(const double latitude, const std::array<double, 12>& monthly_solar_declination) {
        std::array<double, 12> solar_height_factors = {};
        size_t i = 0;
        for (double solar_declination : monthly_solar_declination) {
    ...
    std::array<double, 12> calculate_monthly_incident_irradiance_solar_gains_south(const std::array<double, 12>& monthly_solar_gain_ratios_south, const std::array<int, 12>& monthly_epc_solar_irradiances) {
        std::array<double, 12> monthly_incident_irradiances_solar_gains_south;
    ...
    std::array<double, 12> calculate_monthly_incident_irradiance_solar_gains_north(const std::array<double, 12>& monthly_solar_gain_ratios_north, const std::array<int, 12>& monthly_epc_solar_irradiances) {
        std::array<double, 12> monthly_incident_irradiances_solar_gains_north;
        for (size_t i = 0; i < 12; ++i) {
    ...
    std::array<double, 12> calculate_monthly_solar_gain_ratios_south(const std::array<double, 12>& monthly_solar_height_factors) {
        const double pf_sg = std::sin(PI / 180 * 90 / 2); // Assume windows are vertical, so no in roof windows
        const double asg_s = ax3bx2cxd(-0.66f, -0.106f, 2.93f, 0, pf_sg);
        const double bsg_s = ax3bx2cxd(3.63f, -0.374f, -7.4f, 0, pf_sg);
        const double csg_s = ax3bx2cxd(-2.71f, -0.991f, 4.59f, 1, pf_sg);

        std::array<double, 12> monthly_solar_gain_ratios_south = {};
        size_t i = 0;
        for (double solar_height_factor : monthly_solar_height_factors) {
    ...
    std::array<double, 12> calculate_monthly_solar_gain_ratios_north(const std::array<double, 12>& monthly_solar_height_factors) {
        const double pf_sg = std::sin(PI / 180 * 90 / 2); // Assume windows are vertical, so no in roof windows
        const double asg_n = ax3bx2cxd(26.3f, -38.5f, 14.8f, 0, pf_sg);
        const double bsg_n = ax3bx2cxd(-16.5f, 27.3f, -11.9f, 0, pf_sg);
        const double csg_n = ax3bx2cxd(-1.06f, -0.0872f, -0.191f, 1, pf_sg);

        std::array<double, 12> monthly_solar_gain_ratios_north = {};
        size_t i = 0;
        for (double solar_height_factor : monthly_solar_height_factors) {
    ...
    std::array<double, 12> calculate_monthly_solar_gains_south(const std::array<double, 12>& incident_irradiance_solar_gains_south, const double solar_gain_house_factor) {
        std::array<double, 12> monthly_solar_gains_south = {};
        size_t i = 0;
        for (double incident_irradiance_solar_gain_south : incident_irradiance_solar_gains_south) {
    ...
    std::array<double, 12> calculate_monthly_solar_gains_north(const std::array<double, 12>& incident_irradiance_solar_gains_north, const double solar_gain_house_factor) {
        std::array<double, 12> monthly_solar_gains_north = {};
        size_t i = 0;
        for (double incident_irradiance_solar_gain_north : incident_irradiance_solar_gains_north) {
    ...
    double calculate_average_daily_hot_water_volume(const int num_occupants) {
        const double showers_vol = (0.45f * num_occupants + 0.65f) * 28.8f;  // Litres, 28.8 equivalent of Mixer with TES
        const double bath_vol = (0.13f * num_occupants + 0.19f) * 50.8f;  // Assumes shower is present
        const double other_vol = 9.8f * num_occupants + 14;
    ...
    double calculate_epc_body_gain(const double house_size) {
        const double epc_num_occupants = 1 + 1.76f * (1 - std::exp(-0.000349f *
    ...
    std::array<double, 12> calculate_monthly_epc_outside_temperatures(int region_identifier) {
        constexpr std::array<std::array<double, 12>, 21> monthly_epc_outside_temperatures_per_region = { {
    ...
    ThermalTransmittanceAndOptimisedEpcDemand calculate_dwellings_thermal_transmittance(const double house_size, const double epc_body_gain, const std::array<double, 12>& monthly_epc_outside_temperatures, const std::array<int, 12>& monthly_epc_solar_irradiances, const std::array<double, 12>& monthly_solar_height_factors, const std::array<double, 12>& monthly_solar_declinations, const std::array<double, 12>& monthly_solar_gains_south, const std::array<double, 12>& monthly_solar_gains_north, const double heat_capacity, const int epc_space_heating) {
        double thermal_transmittance = 0.5;
        double optimised_epc_demand = 0;

        constexpr std::array<size_t, 12> days_in_months = { 31, 28, 31, 30, 31, 30, 31, 31, 30, 31, 30, 31 };
        static constexpr std::array<double, 24> summer_temperature_profile = { 7, 7, 7, 7, 7, 7, 7, 7, 7, 7, 7, 7, 7, 7, 7, 7, 7, 7, 7, 7, 7, 7, 7, 7 };
        static constexpr std::array<double, 24> weekend_temperature_profile = { 7, 7, 7, 7, 7, 7, 7, 20, 20, 20, 20, 20, 20, 20, 20, 20, 20, 20, 20, 20, 20, 20, 20, 20 };
        static constexpr std::array<double, 24> default_temperature_profile = { 7, 7, 7, 7, 7, 7, 7, 20, 20, 20, 7, 7, 7, 7, 7, 7, 20, 20, 20, 20, 20, 20, 20, 20 };

        // thermal_transmittance_current == ttc
        for (double thermal_transmittance_current = 0.5f; thermal_transmittance_current < 3.0f; thermal_transmittance_current += 0.01f) {
            int month = 0;
            double inside_temperature_current = 20;  // Initial temperature
            double epc_demand = 0;

            for (size_t days_in_month : days_in_months) {
                const double outside_temperature_current = monthly_epc_outside_temperatures.at(month);
                const int solar_irradiance_current = monthly_epc_solar_irradiances.at(month);
                const double solar_height_factor = monthly_solar_height_factors.at(month);
                const double solar_declination_current = monthly_solar_declinations.at(month);
                const double solar_gain_south = monthly_solar_gains_south.at(month);
                const double solar_gain_north = monthly_solar_gains_north.at(month);

                for (size_t day = 0; day < days_in_month; ++day) {
                    const std::array<double, 24>& hourly_temperature_profile = select_hourly_epc_temperature_profile(month, day, summer_temperature_profile, weekend_temperature_profile, default_temperature_profile);

                    for (size_t hour = 0; hour < 24; ++hour) {
                        const double desired_temperature_current = hourly_temperature_profile.at(hour);
                        const double heat_flow_out = (house_size * thermal_transmittance_current * (inside_temperature_current - outside_temperature_current)) / 1000;
                        ...
                        if (inside_temperature_current < desired_temperature_current) {  //  Requires heating
                            const double space_hr_demand = (desired_temperature_current - inside_temperature_current) * heat_capacity;
            ...
            const double epc_optimal_heating_demand_diff = std::abs(epc_space_heating - optimised_epc_demand);
            const double epc_heating_demand_diff = std::abs(epc_space_heating - epc_demand);
    ...
    Demand calculate_yearly_space_and_hot_water_demand(const std::array<double, 24>& hourly_temperatures_over_day, const double thermostat_temperature, const std::array<double, 12>& hot_water_monthly_factors, const std::array<double, 12>& monthly_cold_water_temperatures, const std::array<double, 12>& monthly_solar_gain_ratios_north, const std::array<double, 12>& monthly_solar_gain_ratios_south, const std::array<double, 24>& dhw_hourly_ratios, const std::vector<double>& hourly_outside_temperatures_over_year, const std::vector<double>& hourly_solar_irradiances_over_year, const double average_daily_hot_water_volume, const int hot_water_temperature, const double solar_gain_house_factor, const double house_size, const double dwelling_thermal_transmittance, const double heat_capacity, const double body_heat_gain) {
        ...
        double max_hourly_demand = 0;
        double demand_total = 0;

        double inside_temperature_current = thermostat_temperature;
        double hot_water_total = 0;

        constexpr std::array<size_t, 12> days_in_months = { 31, 28, 31, 30, 31, 30, 31, 31, 30, 31, 30, 31 };

        size_t month = 0;
        for (size_t days_in_month : days_in_months) {
            const double hot_water_monthly_factor = hot_water_monthly_factors.at(month);
            const double cold_water_temperature = monthly_cold_water_temperatures.at(month);
            const double ratio_solar_gain_south = monthly_solar_gain_ratios_south.at(month);
            const double ratio_solar_gain_north = monthly_solar_gain_ratios_north.at(month);
            for (size_t day = 0; day < days_in_month; ++day) {
                for (size_t hour = 0; hour < 24; ++hour) {
        ...
        const double space_demand = demand_total - hot_water_total;
        std::cout << "Yearly Hot Water Demand: " + double_to_string(hot_water_total, 4) << +" kWh\n";
        std::cout << "Yearly Space demand: " + double_to_string(space_demand, 4) << +" kWh\n";
        std::cout << "Yearly Total demand: " + double_to_string(demand_total, 4) << +" kWh\n";
        std::cout << "Max hourly demand: " + double_to_string(max_hourly_demand, 4) << +" kWh\n";
    ...
    void calculate_hourly_space_and_hot_water_demand(const std::array<double, 24>& hourly_temperatures_over_day, double& inside_temp_current, const double ratio_solar_gain_south, const double ratio_solar_gain_north, const double cwt_current, const double dhw_mf_current, double& demand_total, double& dhw_total, double& max_hourly_demand, const size_t hour_year_counter, const size_t hour, const std::array<double, 24>& dhw_hourly_ratios, const std::vector<double>& hourly_outside_temperatures_over_year, const std::vector<double>& hourly_solar_irradiances_over_year, const double average_daily_hot_water_volume, const int hot_water_temperature, const double solar_gain_house_factor, const double house_size, const double dwelling_thermal_transmittance, const double heat_capacity, const double body_heat_gain) {
        const double desired_temp_current = hourly_temperatures_over_day.at(hour);
        const double dhw_hr_current = dhw_hourly_ratios.at(hour);
        const double outside_temp_current = hourly_outside_temperatures_over_year.at(hour_year_counter);
        const double solar_irradiance_current = hourly_solar_irradiances_over_year.at(hour_year_counter);

        const double dhw_hr_demand = (average_daily_hot_water_volume * 4.18f * (hot_water_temperature - cwt_current) / 3600) * dhw_mf_current * dhw_hr_current;

        const double incident_irradiance_solar_gain_south = solar_irradiance_current * ratio_solar_gain_south;
        const double incident_irradiance_solar_gain_north = solar_irradiance_current * ratio_solar_gain_north;
        const double solar_gain_south = incident_irradiance_solar_gain_south * solar_gain_house_factor;
        const double solar_gain_north = incident_irradiance_solar_gain_north * solar_gain_house_factor;

        //double solar_irradiance_current = Solar_Irradiance[Weather_Count]
        const double heat_loss = (house_size * dwelling_thermal_transmittance * (inside_temp_current - outside_temp_current)) / 1000;

        // heat_flow_out in kWh, +ve means heat flows out of building, -ve heat flows into building
        inside_temp_current += (-heat_loss + solar_gain_south + solar_gain_north + body_heat_gain) / heat_capacity;

        double space_hr_demand = 0;
        ...
        const double hourly_demand = dhw_hr_demand + space_hr_demand;
    ...
    std::vector<size_t> linearly_space(double range, size_t segments) {
        ...
        int j = 0;
        for (double i = 0; j < range; i += step) {
            j = static_cast<int>(i);
            if (static_cast<double>(i - j) > 0.5f) ++j;
            points.push_back(j);
            //std::cout << i << ", " << j << '\n';
        }
        return points;
    ...
    double get_or_calculate(const size_t i, const size_t j, const size_t x_size, double& min_z, std::vector<double>& zs,
        ...
        constexpr double unset_z = 3.40282e+038f;
        double& z = zs.at(i + j * x_size);
        if (z == unset_z) {
    ...
    void if_unset_calculate(const size_t i, const size_t j, const size_t x_size, double& min_z, std::vector<double>& zs,
        ...
        if (zs.at(i + j * x_size) == unset_z) {
            ...
            double z = calculate_optimal_tariff(hp_option, solar_option, static_cast<int>(j), optimum_tes_npc, solar_maximum, static_cast<int>(i), cop_worst, hp_electrical_power, ground_temp, optimal_spec, temp_profile, thermostat_temperature, hot_water_temperature, cumulative_discount_rate, monthly_solar_gain_ratios_north, monthly_solar_gain_ratios_south, monthly_cold_water_temperatures, dhw_monthly_factors, monthly_solar_declinations, monthly_roof_ratios_south, hourly_outside_temperatures_over_year, hourly_solar_irradiances_over_year, u_value, heat_capacity, agile_tariff_per_hour_over_year, hot_water_hourly_ratios, average_daily_hot_water_volume, grid_emissions, solar_gain_house_factor, body_heat_gain, house_size_thermal_transmittance_product);
            // may not need min_z
            if (z < min_z) min_z = z;
        }
    }
    ...
    void simulate_heat_solar_combination(const HeatOption hp_option, const SolarOption solar_option, const int solar_maximum, const int tes_range, const double ground_temp, HeatSolarSystemSpecifications& optimal_spec, const std::array<double, 24>& erh_hourly_temperatures_over_day, const std::array<double, 24>& hp_hourly_temperatures_over_day, const int hot_water_temperature, const double coldest_outside_temperature_of_year, const double maximum_hourly_erh_demand, const double maximum_hourly_hp_demand, const double thermostat_temperature, const double cumulative_discount_rate, const std::array<double, 12>& monthly_solar_gain_ratios_north, const std::array<double, 12>& monthly_solar_gain_ratios_south, const std::array<double, 12>& monthly_cold_water_temperatures, const std::array<double, 12>& dhw_monthly_factors, const std::array<double, 12>& monthly_solar_declinations, const std::array<double, 12>& monthly_roof_ratios_south, const std::vector<double>& hourly_outside_temperatures_over_year, const std::vector<double>& hourly_solar_irradiances_over_year, const double u_value, const double heat_capacity, const std::vector<double>& agile_tariff_per_hour_over_year, const std::array<double, 24>& hot_water_hourly_ratios, const double average_daily_hot_water_volume, const int grid_emissions, const double solar_gain_house_factor, const double body_heat_gain, const double house_size_thermal_transmittance_product) {
        ...
        const std::array<double, 24>& temp_profile = select_temp_profile(hp_option, hp_hourly_temperatures_over_day, erh_hourly_temperatures_over_day);
        const double cop_worst = calculate_cop_worst(hp_option, hot_water_temperature, coldest_outside_temperature_of_year);
        const double hp_electrical_power = calculate_hp_electrical_power(hp_option, maximum_hourly_erh_demand, maximum_hourly_hp_demand, cop_worst);
        const int solar_size_range = calculate_solar_size_range(solar_option, solar_maximum);
        double optimum_tes_npc = 3.40282e+038f;

        // OPTIMISER ==========================================================================================
        const size_t min_step = 3;
        double gradient_factor = 0.15f;
        int target_step = 7;
        ...
        if (y_size > 1 && simulation_options.use_optimisation_surfaces) {
            // non-user variables
            constexpr double unset_z = 3.40282e+038f; // if z has no been found yet it is set to max double value
            double min_z = unset_z; // record the current minimum z
            double max_mx = 0, max_my = 0; // gradient of steepest segment

            // create blank surface of z's
            std::vector<double> zs(x_size * y_size, unset_z);
            ...
            std::vector<size_t> is = linearly_space(static_cast<double>(x_size - 1), x_subdivisions);
            std::vector<size_t> js = linearly_space(static_cast<double>(y_size - 1), y_subdivisions);
            ...
            for (IndexRect& r : index_rects) {
                const double z11 = get_or_calculate(r.i1, r.j1, x_size, min_z, zs, hp_option, solar_option, optimum_tes_npc, solar_maximum, cop_worst, hp_electrical_power, ground_temp, optimal_spec, &temp_profile, thermostat_temperature, hot_water_temperature, cumulative_discount_rate, monthly_solar_gain_ratios_north, monthly_solar_gain_ratios_south, monthly_cold_water_temperatures, dhw_monthly_factors, monthly_solar_declinations, monthly_roof_ratios_south, hourly_outside_temperatures_over_year, hourly_solar_irradiances_over_year, u_value, heat_capacity, agile_tariff_per_hour_over_year, hot_water_hourly_ratios, average_daily_hot_water_volume, grid_emissions, solar_gain_house_factor, body_heat_gain, house_size_thermal_transmittance_product);
                const double z21 = get_or_calculate(r.i2, r.j1, x_size, min_z, zs, hp_option, solar_option, optimum_tes_npc, solar_maximum, cop_worst, hp_electrical_power, ground_temp, optimal_spec, &temp_profile, thermostat_temperature, hot_water_temperature, cumulative_discount_rate, monthly_solar_gain_ratios_north, monthly_solar_gain_ratios_south, monthly_cold_water_temperatures, dhw_monthly_factors, monthly_solar_declinations, monthly_roof_ratios_south, hourly_outside_temperatures_over_year, hourly_solar_irradiances_over_year, u_value, heat_capacity, agile_tariff_per_hour_over_year, hot_water_hourly_ratios, average_daily_hot_water_volume, grid_emissions, solar_gain_house_factor, body_heat_gain, house_size_thermal_transmittance_product);
                const double z22 = get_or_calculate(r.i2, r.j2, x_size, min_z, zs, hp_option, solar_option, optimum_tes_npc, solar_maximum, cop_worst, hp_electrical_power, ground_temp, optimal_spec, &temp_profile, thermostat_temperature, hot_water_temperature, cumulative_discount_rate, monthly_solar_gain_ratios_north, monthly_solar_gain_ratios_south, monthly_cold_water_temperatures, dhw_monthly_factors, monthly_solar_declinations, monthly_roof_ratios_south, hourly_outside_temperatures_over_year, hourly_solar_irradiances_over_year, u_value, heat_capacity, agile_tariff_per_hour_over_year, hot_water_hourly_ratios, average_daily_hot_water_volume, grid_emissions, solar_gain_house_factor, body_heat_gain, house_size_thermal_transmittance_product);
                const double z12 = get_or_calculate(r.i1, r.j2, x_size, min_z, zs, hp_option, solar_option, optimum_tes_npc, solar_maximum, cop_worst, hp_electrical_power, ground_temp, optimal_spec, &temp_profile, thermostat_temperature, hot_water_temperature, cumulative_discount_rate, monthly_solar_gain_ratios_north, monthly_solar_gain_ratios_south, monthly_cold_water_temperatures, dhw_monthly_factors, monthly_solar_declinations, monthly_roof_ratios_south, hourly_outside_temperatures_over_year, hourly_solar_irradiances_over_year, u_value, heat_capacity, agile_tariff_per_hour_over_year, hot_water_hourly_ratios, average_daily_hot_water_volume, grid_emissions, solar_gain_house_factor, body_heat_gain, house_size_thermal_transmittance_product);

                const double mx = std::abs((z11 - z21) / (r.i2 - r.i1));
                const double my = std::abs((z11 - z12) / (r.j2 - r.j1));
            ...
            while (!index_rects.empty()) {
                ...
                for (IndexRect& r : index_rects) {
                    ...
                    double min_local_z = min_4f(z11, z21, z22, z12);
                    // estimate minimum npc between nodes
                    double min_z_estimate = min_local_z - (max_mx * di + max_my * dj);
            ...
            if (false) {
                ...
                for (size_t j = 0; j < y_size; ++j) {
                    for (size_t i = 0; i < x_size; ++i) {
                        const double z = zs.at(i + j * x_size);
                        if (z == unset_z) {
                            //std::cout << "-";
                        }
                        else {
                ...
                const double efficiency = (static_cast<double>(points_searched) / static_cast<double>(zs.size())) * 100.0f;
    ...
    double calculate_cumulative_discount_rate(const double discount_rate, const int npc_years) {
        double discount_rate_current = 1;
        double cumulative_discount_rate = 0;
    ...
    std::array<double, 12> calculate_roof_ratios_south(const std::array<double, 12>& monthly_solar_declinations, const double latitude) {
        const double pf = std::sin(PI / 180 * 35 / 2);  // Assume roof is 35° from horizontal
        const double a = ax3bx2cxd(-0.66f, -0.106f, 2.93f, 0, pf);
        const double b = ax3bx2cxd(3.63f, -0.374f, -7.4f, 0, pf);
        const double c = ax3bx2cxd(-2.71f, -0.991f, 4.59f, 1, pf);

        std::array<double, 12> ratios_roof_south = {};
        size_t month = 0;
        for (const double& solar_declination_current : monthly_solar_declinations) {
            const double solar_height_factor = std::cos(PI / 180 * (latitude - solar_declination_current));
    ...
    double calculate_optimal_tariff(const HeatOption hp_option, const SolarOption solar_option, const int solar_size, double& optimum_tes_npc, const int solar_maximum, const int tes_option, const double cop_worst, const double hp_electrical_power, const double ground_temp, HeatSolarSystemSpecifications& optimal_spec, const std::array<double, 24>* temp_profile, const double thermostat_temperature, const int hot_water_temperature, const double cumulative_discount_rate, const std::array<double, 12>& monthly_solar_gain_ratios_north, const std::array<double, 12>& monthly_solar_gain_ratios_south, const std::array<double, 12>& monthly_cold_water_temperatures, const std::array<double, 12>& dhw_monthly_factors, const std::array<double, 12>& monthly_solar_declinations, const std::array<double, 12>& monthly_roof_ratios_south, const std::vector<double>& hourly_outside_temperatures_over_year, const std::vector<double>& hourly_solar_irradiances_over_year, const double u_value, const double heat_capacity, const std::vector<double>& agile_tariff_per_hour_over_year, const std::array<double, 24>& hot_water_hourly_ratios, const double average_daily_hot_water_volume, const int grid_emissions, const double solar_gain_house_factor, const double body_heat_gain, const double house_size_thermal_transmittance_product) {
        ...
        double tes_volume_current = 0.1f + tes_option * 0.1f; // m3
        const double hp_electrical_power_worst = hp_electrical_power * cop_worst; // hp option
        const double capex = calculate_capex_heatopt(hp_option, hp_electrical_power_worst) + calculate_capex_pv(solar_option, pv_size) + calculate_capex_solar_thermal(solar_option, solar_thermal_size) + calculate_capex_tes_volume(tes_volume_current);

        const double tes_radius = std::pow((tes_volume_current / (2 * PI)), (1.0f / 3.0f));  //For cylinder with height = 2x radius
        const double tes_charge_full = tes_volume_current * 1000 * 4.18f * (hot_water_temperature - 40) / 3600; // 40 min temp
        const double tes_charge_boost = tes_volume_current * 1000 * 4.18f * (60 - 40) / 3600; //  # kWh, 60C HP with PV boost
        const double tes_charge_max = tes_volume_current * 1000 * 4.18f * (95 - 40) / 3600; //  # kWh, 95C electric and solar

        const double tes_charge_min = 10 * 4.18f * (hot_water_temperature - 10) / 3600; // 10litres hot min amount
        ...
        double optimum_tariff = 1000000;
        double min_npc = 1000000;
        for (int tariff_int = 0; tariff_int < 5; ++tariff_int) {
            Tariff tariff = static_cast<Tariff>(tariff_int);
            size_t hour_year_counter = 0;
            //std::cout << "here\n";
            double inside_temp_current = thermostat_temperature;  // Initial temp
            double solar_thermal_generation_total = 0;
            double operational_costs_peak = 0;
            double operational_costs_off_peak = 0;
            double operation_emissions = 0;

            double tes_state_of_charge = tes_charge_full;  // kWh, for H2O, starts full to prevent initial demand spike
            // https ://www.sciencedirect.com/science/article/pii/S0306261916302045

            int month = 0;
            for (int days_in_month : days_in_months) {
                double ratio_sg_south = monthly_solar_gain_ratios_south.at(month);
                double ratio_sg_north = monthly_solar_gain_ratios_north.at(month);
                double cwt_current = monthly_cold_water_temperatures.at(month);
                double dhw_mf_current = dhw_monthly_factors.at(month);

                const double solar_declination_current = monthly_solar_declinations.at(month);
                double ratio_roof_south = monthly_roof_ratios_south.at(month);
            ...
            const double total_operational_cost = operational_costs_peak + operational_costs_off_peak; // tariff
            const double npc = capex + total_operational_cost * cumulative_discount_rate;

            if (simulation_options.output_all_specs) {
                const double net_present_cost_current = capex + total_operational_cost * cumulative_discount_rate;
            ...
            if (total_operational_cost < optimum_tariff) {
                ...
                const double net_present_cost_current = capex + total_operational_cost * cumulative_discount_rate; // £s
    ...
    void calculate_inside_temp_change(double& inside_temp_current, const double outside_temp_current, const double solar_irradiance_current, const double ratio_sg_south, const double ratio_sg_north, const double ratio_roof_south, const double solar_gain_house_factor, const double body_heat_gain, const double house_size_thermal_transmittance_product, const double heat_capacity) {
        const double incident_irradiance_sg_s = solar_irradiance_current * ratio_sg_south;
        const double incident_irradiance_sg_n = solar_irradiance_current * ratio_sg_north;
        const double solar_gain_south = incident_irradiance_sg_s * solar_gain_house_factor;
        const double solar_gain_north = incident_irradiance_sg_n * solar_gain_house_factor;

        //double solar_irradiance_current = Solar_Irradiance[Weather_Count]
        //const double heat_loss = (house_size * thermal_transmittance * (inside_temp_current - outside_temp_current)) / 1000;
        const double heat_loss = house_size_thermal_transmittance_product * (inside_temp_current - outside_temp_current);

        // heat_flow_out in kWh, +ve means heat flows out of building, -ve heat flows into building
        inside_temp_current += (-heat_loss + solar_gain_south + solar_gain_north + body_heat_gain) / heat_capacity;
    }

    TesTempAndHeight::TesTempAndHeight(const double upper_temperature, const double lower_temperature, const double thermocline_height)
    ...
    double TesTempAndHeight::clamp_height(const double height) {
        if (height < 0) {
            return 0;
        }
        else if (height > 1) {
    ...
    double calculate_solar_thermal_generation_current(const SolarOption solar_option, const double tes_upper_temperature, const double tes_lower_temperature, const int solar_thermal_size, const double incident_irradiance_roof_south, const double outside_temp_current) {
        double solar_thermal_generation_current = 0; // if solar_option < 2
        if (solar_option >= SolarOption::FP) {
            const double solar_thermal_collector_temperature = (tes_upper_temperature + tes_lower_temperature) / 2;
            ...
            if (incident_irradiance_roof_south == 0) {
                ...
                return std::max(0.8f * solar_thermal_size * ax2bxc(a, b, c * incident_irradiance_roof_south, solar_thermal_collector_temperature - outside_temp_current), 0.0);
            }
        }
        else {
            return 0;
    ...
    double calculate_hourly_space_demand(double& inside_temp_current, const double desired_min_temp_current, const double cop_current, const double tes_state_of_charge, const double dhw_hr_demand, const double hp_electrical_power, const double heat_capacity) {
        if (inside_temp_current > desired_min_temp_current) {
            ...
            double space_hr_demand = (desired_min_temp_current - inside_temp_current) * heat_capacity;
    ...
    double calculate_electrical_demand_for_heating(double& tes_state_of_charge, const double space_water_demand, const double hp_electrical_power, const double cop_current) {
        if (space_water_demand < tes_state_of_charge) { // TES can provide all demand
            ...
            if (tes_state_of_charge > 0) {
                const double electrical_demand_current = (space_water_demand - tes_state_of_charge) / cop_current;;
    ...
    void boost_tes_and_electrical_demand(double& tes_state_of_charge, double& electrical_demand_current, const double pv_remaining_current, const double tes_charge_boost, const double hp_electrical_power, const double cop_boost) {
        ...
        const double tes_boost_state_charge_diff = tes_charge_boost - tes_state_of_charge;
    ...
    void simulate_heating_system_for_day(const std::array<double, 24>* temp_profile, double& inside_temp_current, const double ratio_sg_south, const double ratio_sg_north, const double cwt_current, double dhw_mf_current, double& tes_state_of_charge, const double tes_charge_full, const double tes_charge_boost, const double tes_charge_max, const double tes_radius, const double ground_temp, const HeatOption hp_option, const SolarOption solar_option, const int pv_size, const int solar_thermal_size, const double hp_electrical_power, const Tariff tariff, double& operational_costs_peak, double& operational_costs_off_peak, double& operation_emissions, double& solar_thermal_generation_total, const double ratio_roof_south, const double tes_charge_min, size_t& hour_year_counter, const std::vector<double>& hourly_outside_temperatures_over_year, const std::vector<double>& hourly_solar_irradiances_over_year, const double u_value, const double heat_capacity, const std::vector<double>& agile_tariff_per_hour_over_year, const std::array<double, 24>& hot_water_hourly_ratios, const double average_daily_hot_water_volume, const int hot_water_temperature, const int grid_emissions, const double solar_gain_house_factor, const double body_heat_gain, const double house_size_thermal_transmittance_product) {
        const double pi_d = PI * tes_radius * 2;
        const double pi_r2 = PI * tes_radius * tes_radius;
        const double pi_d2 = pi_d * tes_radius * 2;
        ...
        for (size_t hour = 0; hour < 24; ++hour) {
            ...
            const double tes_upper_losses = (tes_upper_temperature - inside_temp_current) * u_value * (pi_d2 * tes_thermocline_height + pi_r2); // losses in kWh
            const double tes_lower_losses = (tes_lower_temperature - inside_temp_current) * u_value * (pi_d2 * (1 - tes_thermocline_height) + pi_r2);
            const double total_losses = tes_upper_losses + tes_lower_losses;
            tes_state_of_charge -= total_losses;
            inside_temp_current += total_losses / heat_capacity;

            const double desired_min_temp_current = temp_profile->at(hour);
            const double agile_tariff_current = agile_tariff_per_hour_over_year.at(hour_year_counter);
            const double dhw_hr_current = hot_water_hourly_ratios.at(hour);
            const double dhw_hr_demand = (average_daily_hot_water_volume * 4.18f * (hot_water_temperature - cwt_current) / 3600) * dhw_mf_current * dhw_hr_current;
            ...
            const double pv_efficiency = calculate_pv_efficiency(solar_option, tes_upper_temperature, tes_lower_temperature);

            const double incident_irradiance_roof_south = solar_irradiance_current * ratio_roof_south / 1000; // kW / m2
            double pv_generation_current = pv_size * pv_efficiency * incident_irradiance_roof_south * 0.8f;  // 80 % shading factor

            const double solar_thermal_generation_current = calculate_solar_thermal_generation_current(solar_option, tes_upper_temperature, tes_lower_temperature, solar_thermal_size, incident_irradiance_roof_south, outside_temp_current);
            ...
            const double space_hr_demand = calculate_hourly_space_demand(inside_temp_current, desired_min_temp_current, cop_current, tes_state_of_charge, dhw_hr_demand, hp_electrical_power, heat_capacity);

            double electrical_demand_current = calculate_electrical_demand_for_heating(tes_state_of_charge, space_hr_demand + dhw_hr_demand, hp_electrical_power, cop_current);
            ...
            const double pv_remaining_current = pv_generation_current - electrical_demand_current;
            ...
            double pv_equivalent_revenue;
            double electrical_import;
    ...
    void print_optimal_specifications(const std::array<HeatSolarSystemSpecifications, 21>& optimal_specifications, const int double_print_precision) {
        ...
        for (const auto& s : optimal_specifications) {
            std::cout << heat_opt_names.at(static_cast<int>(s.heat_option)) << ", " << solar_opt_names.at(static_cast<int>(s.solar_option)) << ", " << s.pv_size << ", " << s.solar_thermal_size << ", " << s.tes_volume << ", " << double_to_string(s.operational_expenditure, double_print_precision) << ", " << double_to_string(s.capital_expenditure, double_print_precision) << ", " << double_to_string(s.net_present_cost, double_print_precision) << ", " << double_to_string(s.operation_emissions, double_print_precision) << ", " << tariff_names.at(static_cast<int>(s.tariff)) << "\n";
        }
    }
    ...
    void write_optimal_specification(const HeatSolarSystemSpecifications& spec, std::ofstream& file) {
            file << static_cast<int>(spec.heat_option) << "," <<
                ...
                double_to_string(spec.tes_volume, 1) << "," <<
                static_cast<int>(spec.tariff) << "," <<
                double_to_string(spec.operational_expenditure, 2) << "," <<
                double_to_string(spec.capital_expenditure, 2) << "," <<
                double_to_string(spec.net_present_cost, 2) << "," <<
                double_to_string(spec.operation_emissions, 2) << "\n";
    }
    ...
    std::string output_to_javascript(const std::array<HeatSolarSystemSpecifications, 21>& optimal_specifications) {
        ...
        for (const auto& s : optimal_specifications) {
            if (i != 0) ss << ',';
            ss << "[\"" << heat_opt_names.at(static_cast<int>(s.heat_option)) << "\", \"" << solar_opt_names.at(static_cast<int>(s.solar_option)) << "\", " << s.pv_size << ", " << s.solar_thermal_size << ", " << s.tes_volume << ", " << double_to_string(s.operational_expenditure, 0) << ", " << double_to_string(s.capital_expenditure, 0) << ", " << double_to_string(s.net_present_cost, 0) << ", " << double_to_string(s.operation_emissions / 1000, 0) << "]";
            ++i;
        }
        ss << ']';
        return ss.str();
