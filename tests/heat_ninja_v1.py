...
#     builtins.print(f"{caller.filename}:{caller.lineno} {elapsed_time:.1f}s : ", end='')
#     builtins.print(*args, **kwargs)
#     if elapsed_time > 15:
#         sys.exit(0)

# JACK KIRBY EDIT STOP
...
# INPUTS
Occupants_Num = 2  # Avg 2
Location_Input = "CV4 7AL"  # TR1 2ES, BH14 0EY, CV4 7AL, M1 4HF, FY8 2JG, IV1 1SN, HS2 9AG, needs to be capitals
EPC_Space_Heating = 3000  # Annual space heating in kWh on EPC, LE10 9330
House_Size = 60.0  # Avg 87, small 60, LE10 85
TES_Volume_Maximum = 0.5  # m3 minimum 0.1, recommended max 3.0
Temp = 20.0  # Thermostat set point
...
# print(EPC_Outside_Temp, EPC_Solar_Irradiance)
...
def function_epc_day_calculation():
    ...
    for h in range(24):
        ...
        # print(ratio_sg_south, Asg_s, Bsg_s, Csg_s, solar_height_factor)
        ...
        # print("%.2f" % outside_temp_current, "%.2f" % solar_irradiance_current, "%.2f" % solar_height_factor,
        #          "%.2f" % solar_declination_current, "%.4f" % solar_gain_s, "%.2f" % solar_gain_n)

        # print(solar_height_factor, "%.4f" % ratio_sg_south, "%.4f" % incident_irradiance_sg_s, "%.4f" % solar_gain_s)
        ...
        if Inside_Temp_Current > desired_temp_current:  # Warm enough already, NO heating required
            ...
            # print(f'i{Inside_Temp_Current}')

        EPC_Demand += (space_hr_demand / 0.9)
        # print(EPC_Demand)
        # print("%.4f" % heat_flow_out, "%.4f" % Inside_Temp_Current, "%.4f" % EPC_Demand)
    EPC_Day += 1


print(f'{EPC_Outside_Temp = }, \n'
      f'{EPC_Solar_Irradiance = }')
...
for Iteration in range(251):  # 0.5 to 3.0
    ...
    for m in range(12):  # EPC LOOP
        if m == 3 or m == 5 or m == 8 or m == 10:  # Months with 30 days -1, as for loop starts at 0
            ...
            for d in range(31):
                function_epc_day_calculation()
        # print(EPC_Demand)
    # print(EPC_Space_Heating, Optimised_EPC_Demand, EPC_Demand)
    ...
    # print("%.2f" % EPC_Demand, "%.2f" % Optimised_EPC_Demand, "%.2f" % Thermal_Transmittance_Current)
    Thermal_Transmittance_Current += 0.01

print("Dwelling thermal transmittance", "%.8f" % Thermal_Transmittance)
print("Resultant EPC demand", "%.8f" % Optimised_EPC_Demand, "kWh")
...
def function_heater_day_calculation():
    for h in range(24):
        ...
        # print(space_hr_demand, dhw_hr_demand, TES_State_of_Charge)
        ...
        for Solar_Size in range(int(Solar_Size_Range)):  # 2m2 min up to max
            ...
            for TES_Option in range(int(TES_Range)):
                ...
                for Tariff in range(5):  # 5 options: Flat rate, Economy 7, and Bulb Smart, Octopus EV, Octopus Agile
                    ...
                    total_operational_cost = Operational_Costs_Peak + Operational_Costs_Off_Peak
                    if (Operational_Costs_Peak + Operational_Costs_Off_Peak) < Optimum_Tariff:  # Best for current tech
                        ...
                        if Net_Present_Cost_Current < Optimum_TES_NPC:  # Lowest cost TES & tariff for heating tech
                            ...
                            Current_TES_and_Tariff_Specifications = \
                                [(Operational_Costs_Peak + Operational_Costs_Off_Peak), CapEx, HP_Option, Solar_Option,
                                PV_Size, Solar_Thermal_Size, TES_Volume_Current, Net_Present_Cost_Current,
                                Operation_Emissions]
...
for spec in Optimum_TES_and_Tariff_Specifications:
    print(spec)
...
H2_Grey_Cost = 0.049  # 4.9p/kWh A greener gas grid: What are the options, lowest cost also more expensive than gas
H2_Blue_Cost = 0.093  # 9.3p/kWh A greener gas grid: What are the options, average cost
H2_Green_Cost = 0.184  # 18.4p/kWh A greener gas grid: What are the options, highest cost
...
H2_Grey_Boiler_OpEx = ((Boiler_Demand_Total / 0.9) * H2_Grey_Cost)  # 90% Boiler efficiency
H2_Blue_Boiler_OpEx = ((Boiler_Demand_Total / 0.9) * H2_Blue_Cost)  # 90% Boiler efficiency
H2_Green_Boiler_OpEx = ((Boiler_Demand_Total / 0.9) * H2_Green_Cost)  # 90% Boiler efficiency
H2_Boiler_CapEx = 2000 + EPC_Space_Heating / 25  # £2200-3000 from A greener gas grid: What are the options
if H2_Boiler_CapEx > 3000:
    H2_Boiler_CapEx = 3000
H2_Grey_Boiler_NPC = H2_Blue_Boiler_NPC = H2_Green_Boiler_NPC = H2_Boiler_CapEx  # £s
for Year in range(NPC_Years):  # Optimum for 20 years cost
    H2_Grey_Boiler_NPC += H2_Grey_Boiler_OpEx / (Discount_Rate ** Year)
    H2_Blue_Boiler_NPC += H2_Blue_Boiler_OpEx / (Discount_Rate ** Year)
    H2_Green_Boiler_NPC += H2_Green_Boiler_OpEx / (Discount_Rate ** Year)

Gas_Boiler_CapEx = H2_Boiler_CapEx - 500  # Estimated 500 less
Gas_Boiler_NPC = Gas_Boiler_CapEx
for Year in range(NPC_Years):
    Gas_Boiler_NPC += Gas_Boiler_OpEx / (Discount_Rate ** Year)

H2_Grey_Fuel_Cell_OpEx = (HP_Demand_Total / 0.94) * H2_Grey_Cost  # 55% thermal efficiency, continuous profile
H2_Blue_Fuel_Cell_OpEx = (HP_Demand_Total / 0.94) * H2_Blue_Cost  # 39% electrical efficiency
H2_Green_Fuel_Cell_OpEx = (HP_Demand_Total / 0.94) * H2_Green_Cost  # Equivalent 94% total efficiency
...
H2_Fuel_Cell_CapEx = 12000 + 2068.3 * 0.1 ** 0.553  # 12000 fuel cell + min TES size
# https://www.sciencedirect.com/science/article/pii/S0360319914031383#bib14 lowest cost
H2_Grey_Fuel_Cell_NPC = H2_Blue_Fuel_Cell_NPC = H2_Green_Fuel_Cell_NPC = H2_Fuel_Cell_CapEx * NPC_Years / 10  # 10 life
for Year in range(NPC_Years):  # Optimum for 20 years cost
    H2_Grey_Fuel_Cell_NPC += H2_Grey_Fuel_Cell_OpEx / (Discount_Rate ** Year)
    H2_Blue_Fuel_Cell_NPC += H2_Blue_Fuel_Cell_OpEx / (Discount_Rate ** Year)
    H2_Green_Fuel_Cell_NPC += H2_Green_Fuel_Cell_OpEx / (Discount_Rate ** Year)

# Biomass source https://www.greenmatch.co.uk/blog/2015/02/how-much-does-a-biomass-boiler-cost
Biomass_Fuel_Cost = 0.0411  # 4.11p/kWh
Biomass_OpEx = ((Boiler_Demand_Total / 0.9) * Biomass_Fuel_Cost)  # 90% Boiler efficiency
Biomass_CapEx = 9000 + EPC_Space_Heating / 4  # £10-19k for automatically fed biomass boilers
if Biomass_CapEx > 19000:
    Biomass_CapEx = 19000
Biomass_NPC = Biomass_CapEx  # £s
for Year in range(NPC_Years):  # Optimum for 20 years cost
    Biomass_NPC += Biomass_OpEx / (Discount_Rate ** Year)

# OpEx vs CapEx plot FULL legend
for t in range(len(Optimum_TES_and_Tariff_Specifications)):
    #  REF: Current_TES_and_Tariff_Specifications = [(Operational_Costs_Peak + Operational_Costs_Off_Peak), CapEx,
    #  HP_Option, Solar_Option, PV_Size, Solar_Thermal_Size, TES_Volume_Current, Net_Present_Cost_Current,
    #  Operation_Emissions]
    if Optimum_TES_and_Tariff_Specifications[t][2] == 0:
        Legend_Current = "Electric Heater"
        Colour = "orange"
    elif Optimum_TES_and_Tariff_Specifications[t][2] == 1:
        Legend_Current = "ASHP"
        Colour = "r"
    elif Optimum_TES_and_Tariff_Specifications[t][2] == 2:
        Legend_Current = "GSHP"
        Colour = "forestgreen"
    else:
        Legend_Current = "STANDBY"
        Colour = "m"
    if Optimum_TES_and_Tariff_Specifications[t][3] == 0:  # No ST or PV
        Marker = "o"
    elif Optimum_TES_and_Tariff_Specifications[t][3] == 1:
        Legend_Current += ", PV " + "%.0f" % Optimum_TES_and_Tariff_Specifications[t][4] + "m2"
        Marker = "*"
    elif Optimum_TES_and_Tariff_Specifications[t][3] == 2:
        Legend_Current += ", FP ST " + "%.0f" % Optimum_TES_and_Tariff_Specifications[t][5] + "m2"
        Marker = "1"
    elif Optimum_TES_and_Tariff_Specifications[t][3] == 3:
        Legend_Current += ", ET ST " + "%.0f" % Optimum_TES_and_Tariff_Specifications[t][5] + "m2"
        Marker = "2"
    elif Optimum_TES_and_Tariff_Specifications[t][3] == 4:
        Legend_Current += ", PV " + "%.0f" % Optimum_TES_and_Tariff_Specifications[t][4] + "m2"
        Legend_Current += ", FP ST " + "%.0f" % Optimum_TES_and_Tariff_Specifications[t][5] + "m2"
        Marker = "+"
    elif Optimum_TES_and_Tariff_Specifications[t][3] == 5:
        Legend_Current += ", PV " + "%.0f" % Optimum_TES_and_Tariff_Specifications[t][4] + "m2"
        Legend_Current += ", ET ST " + "%.0f" % Optimum_TES_and_Tariff_Specifications[t][5] + "m2"
        Marker = "x"
    else:  # PVT
        Legend_Current += ", PVT " + "%.0f" % Optimum_TES_and_Tariff_Specifications[t][4] + "m2"
        Marker = "p"
    Legend_Current += " & TES " + ("%.1f" % Optimum_TES_and_Tariff_Specifications[t][6]) + "m3"
    plt.plot(Optimum_TES_and_Tariff_Specifications[t][1]/1000, Optimum_TES_and_Tariff_Specifications[t][0],
             marker=Marker, linestyle="None", color=Colour, label=Legend_Current)
plt.plot(Gas_Boiler_CapEx / 1000, Gas_Boiler_OpEx, "o", color="k", label="Natural Gas Boiler")
plt.plot(H2_Boiler_CapEx / 1000, H2_Grey_Boiler_OpEx, "o", color="grey", label="Grey Hydrogen Boiler")
plt.plot(H2_Boiler_CapEx / 1000, H2_Blue_Boiler_OpEx, "o", color="b", label="Blue Hydrogen Boiler")
plt.plot(H2_Boiler_CapEx / 1000, H2_Green_Boiler_OpEx, "o", color="springgreen", label="Green Hydrogen Boiler")
plt.plot(H2_Fuel_Cell_CapEx / 1000, H2_Grey_Fuel_Cell_OpEx, "s", color="grey", label="Grey Hydrogen Fuel Cell")
plt.plot(H2_Fuel_Cell_CapEx / 1000, H2_Blue_Fuel_Cell_OpEx, "s", color="b", label="Blue Hydrogen Fuel Cell")
plt.plot(H2_Fuel_Cell_CapEx / 1000, H2_Green_Fuel_Cell_OpEx, "s", color="springgreen", label="Green Hydrogen Fuel Cell")
plt.plot(Biomass_CapEx / 1000, Biomass_OpEx, "o", color="chocolate", label="Biomass Boiler")
plt.xlabel("Capital Cost (£1,000s)")
plt.ylabel("Yearly Operational Costs (£)")
plt.legend(frameon=False, loc="upper left", bbox_to_anchor=(1.0, 1.0), prop={"size": 8})
plt.show()

# gCO2e/kWh vs NPC plots REDUCED legend
Demand_NPC = 0  # £s
for Year in range(NPC_Years):  # Optimum for 20 years cost
    Demand_NPC += Boiler_Demand_Total / (Discount_Rate ** Year)
for t in range(len(Optimum_TES_and_Tariff_Specifications)):
    #  REF: Current_TES_and_Tariff_Specifications = [(Operational_Costs_Peak + Operational_Costs_Off_Peak), CapEx,
    #  HP_Option, Solar_Option, PV_Size, Solar_Thermal_Size, TES_Volume_Current, Net_Present_Cost_Current,
    #  Operation_Emissions]
    ...
    if Optimum_TES_and_Tariff_Specifications[t][3] == 0:  # No ST or PV
        ...
        Marker = "p"
    plt.plot(Optimum_TES_and_Tariff_Specifications[t][7] / Demand_NPC,
        Optimum_TES_and_Tariff_Specifications[t][8] / (1000 * Boiler_Demand_Total) * 1000,
        marker=Marker, linestyle="None", color=Colour)
plt.plot(Gas_Boiler_NPC / Demand_NPC, ((Boiler_Demand_Total / 0.9) * 183) / Boiler_Demand_Total,
    marker="o", color="k")  # 183 gCO2e/kWh for UK natural gas
# https://www.gov.uk/government/publications/greenhouse-gas-reporting-conversion-factors-2021
plt.plot(H2_Grey_Boiler_NPC / Demand_NPC, ((Boiler_Demand_Total / 0.9) * 382) / Boiler_Demand_Total,
    marker="o", color="grey")  # 382 middle value from SMR w/o CCS in parliament post
plt.plot(H2_Blue_Boiler_NPC / Demand_NPC, ((Boiler_Demand_Total / 0.9) * 60) / Boiler_Demand_Total,
    marker="o", color="b")  # 60 middle value from SMR with CCS in parliament post
plt.plot(H2_Green_Boiler_NPC / Demand_NPC,
    ((Boiler_Demand_Total / 0.9) * 1875 * (Grid_Emissions / 1000)) / Boiler_Demand_Total, marker="o",
    color="springgreen")  # 1875 gCO2e/kWhe of Grid in parliament post
plt.plot(H2_Grey_Fuel_Cell_NPC / Demand_NPC, ((Boiler_Demand_Total / 0.94) * 382) / Boiler_Demand_Total,
    marker="s", color="grey")  # 382 middle value from SMR w/o CCS in parliament post
plt.plot(H2_Blue_Fuel_Cell_NPC / Demand_NPC, ((Boiler_Demand_Total / 0.94) * 60) / Boiler_Demand_Total,
    marker="s", color="b")  # 60 middle value from SMR with CCS in parliament post
plt.plot(H2_Green_Fuel_Cell_NPC / Demand_NPC,
    ((Boiler_Demand_Total / 0.94) * 1875 * (Grid_Emissions / 1000)) / Boiler_Demand_Total, marker="s",
    color="springgreen")  # 1875 gCO2/kWhe of Grid in post
plt.plot(Biomass_NPC / Demand_NPC, ((Boiler_Demand_Total / 0.9) * 90) / Boiler_Demand_Total,
    marker="o", color="chocolate")  # 90gCO2/kWh middle value from post
# Dummy data for simplified legend
plt.plot(-0.1, -10, "o", color="k", label="HP or Boiler Alone")
plt.plot(-0.1, -10, "*", color="k", label="HP & PV")
plt.plot(-0.1, -10, "1", color="k", label="HP & FP ST")
plt.plot(-0.1, -10, "2", color="k", label="HP & ET ST")
plt.plot(-0.1, -10, "+", color="k", label="HP & PV, FP ST")
plt.plot(-0.1, -10, "x", color="k", label="HP & PV, ET ST")
plt.plot(-0.1, -10, "p", color="k", label="HP & PVT")
plt.plot(-0.1, -10, "s", color="k", label="Fuel Cell")
plt.legend(frameon=False, loc="upper left", bbox_to_anchor=(1.0, 1.0), prop={"size": 8})
plt.legend(loc='upper left', bbox_to_anchor=(1.0, 1.0), prop={"size": 10})
plt.xlabel("20 Year Net Present Cost (£/kWh)")
plt.ylabel("Equivalent Emissions (gCOe/kWh)")
plt.show()

print("Ciao for now")

builtins.print(f"Total Elapsed Time: {time.time() - START_TIME}s")
